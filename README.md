...
**CLI → allowlisted 공식 MCP → visible Chrome → guided manual** 순으로 경로를 낮춥니다.
MCP는 `mcp_allowlist`에 server·transport·publisher·tool·provenance가 모두 일치할 때만 쓰이며
(기본 allowlist는 비어 있어 자동 실행 0), Chrome은 공식 Claude in Chrome 연결 시 visible mode로만
씁니다. 라우터는 경로 결정·검증·다운그레이드까지 하고, 실제 MCP tool 호출은 모델이(안전 게이트가
계속 가드), Chrome 조작·최종 submit은 사용자가 합니다.
...
**CLI → allowlisted official MCP → visible Chrome → guided manual**. MCP is used only when the
`mcp_allowlist` matches on server, transport, publisher, tools, and provenance (the default
allowlist is empty, so nothing auto-runs); Chrome is used in visible mode only when the official
Claude in Chrome is connected. The router decides, validates, and downgrades — actual MCP tool
calls are made by the model (still guarded by the safety gate), and Chrome actions / final
submit are done by the user.
