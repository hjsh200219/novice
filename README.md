# novice — 비개발자 입문자용 Claude Code 플러그인 · a beginner-friendly Claude Code plugin
...
- 스펙 · Spec: `docs/PRD.md` (revision 9)
- 최소 지원 런타임 · Minimum supported runtime: Claude Code `2.1.215`
...
| 명령 · Command | 동작 · Behavior |
|---|---|
| `/novice:mode` | 현재 level, 적용 범위, 안전 게이트 유지 여부 표시<br>*Show current level, scope, and whether the safety gate stays on* |
| `/novice:mode 1` | Level 1 (기본값) — 모든 용어에 설명 병기, 실행 전·후 해설<br>*Level 1 (default) — explanation appended to every term, before/after narration* |
| `/novice:mode 2` | Level 2 — 첫 1회만 설명, 핵심 결정만 해설<br>*Level 2 — explain on first occurrence only, narrate key decisions only* |
| `/novice:mode 3` | Level 3 — 요청 시만 설명, 아키텍처·유저플로우 중심<br>*Level 3 — explain on request only, architecture/user-flow focused* |
| `/novice:mode off` | novice 톤·설명·시각화 완전 제거 (안전 게이트는 유지)<br>*Fully remove novice tone/explanations/visualization (safety gate stays on)* |
...
## 외부 서비스 CLI 부트스트랩 (2-tier) · External service CLI bootstrap
...
- **Tier 1** (검토된 manifest: Vercel, GitHub CLI, Supabase): 고정 package coordinate로
  ...
  *(reviewed manifests: Vercel, GitHub CLI, Supabase) — runs the standard approval flow with
  fixed package coordinates; install and login are each approved once.*
- **Tier 2** (그 외 CLI): 공식 문서 URL·package coordinate·실행 argv를 화면에 그대로
  ...
  *(any other CLI) — shows the official docs URL, package coordinate, and exact argv on
  screen, and proceeds through the same engine only after the user confirms and approves the
  evidence; if no official provenance can be confirmed, it falls back to a guided manual.*
...
*Creating resources, entering env/secret values, and deploying are done by the user directly
or guided manually. The plugin never requests, stores, forwards, or auto-fills credential
values. For a CLI that supports secure storage (gh, supabase), auto-login is aborted when the
environment falls back to plaintext. For a CLI whose documented default is file storage
(vercel), the storage location and logout path are disclosed before the login is approved —
the per-provider policy lives in each manifest's `credential_store`.*

## 안전 게이트 위협 모델 · Safety gate threat model

| 위협 · Threat | 동작 · Behavior | 보증하지 않는 범위 · Not guaranteed |
|---|---|---|
| 로컬 파괴 명령 (`rm -rf`, PowerShell `Remove-Item` 등)<br>*Local destructive commands* | Bash·PowerShell 각각 유한 grammar 해석 후 deny/ask<br>*Parsed by the Bash/PowerShell finite grammars, then deny/ask* | 난독화, 새 명령, 플러그인 밖 실행<br>*Obfuscation, novel commands, execution outside the plugin* |
| Git history 파괴 (`push --force`, `reset --hard`, `clean`)<br>*Git history destruction* | protected branch·범위 기준 deny/ask<br>*deny/ask based on protected branch and scope* | 다른 Git client 작업<br>*Actions from another Git client* |
| DB/원격 리소스 삭제 (CLI·MCP)<br>*DB / remote resource deletion* | 대상·환경 표시 후 ask, production 고위험 deny<br>*ask after showing target and environment, deny for high-risk production* | 외부 콘솔 직접 작업<br>*Actions taken directly in an external console* |
| 시크릿 commit · *Secret commit* | commit candidate tree 스캔 후 deny<br>*scan the commit candidate tree, then deny* | 미지원 포맷, 암호화·난독화된 시크릿<br>*Unsupported formats, encrypted/obfuscated secrets* |
| 시크릿 deploy·출력 노출<br>*Secret deploy / output exposure* | 인자·파일 스캔 deny/ask, 출력 redaction<br>*scan args and files (deny/ask), redact output* | 이미 원격에 저장된 값<br>*Values already stored remotely* |
| 비용·반복 루프 · *Cost / retry loops* | batch당 최대 1회 개입 안내<br>*at most one intervention per batch* | 정확한 과금 계산, hard billing cap<br>*Exact billing, hard billing cap* |

### 명시적 비보증 · Explicit non-guarantees (No silent security claims)
...
- **범용 shell parser가 아닙니다.** Bash와 PowerShell 각각에 대해 단일 command + argv
  ...
  deny, 아니면 ask로 낮춥니다.
  *Not a general shell parser. It parses only a single command + argv finite grammar for Bash
  and PowerShell each (Bash uses backslash escapes; PowerShell uses backtick escapes and
...
- **플러그인을 disable/uninstall하면 안전 게이트도 사라집니다.** "always-on"은 플러그인이
  ...
  *Disabling/uninstalling the plugin removes the safety gate too. "Always-on" only means while
  the plugin is enabled. novice `off` turns off only the learning layer; the safety gate stays on.*
...
- 원격 telemetry를 보내지 않습니다. *No remote telemetry is sent.*

## 개발 · Development

```bash
npm test                  # 전체 · everything (unit + integration)
npm run test:unit
