...
[![npm](https://img.shields.io/npm/v/claude-novice?label=npm%20claude-novice)](https://www.npmjs.com/package/claude-novice)
...
| 위협 | 동작 | 보증하지 않는 범위 |
|---|---|---|
| 로컬 파괴 명령 (`rm -rf ~`·`/`·프로젝트 루트, `dd`/`mkfs`/`shred`, PowerShell `Format-Volume`/`Clear-Disk`) | bare 명령과 제한된 단순 prefix(`NAME=value`, `env`, `command`, `sudo`)를 파싱해 catastrophic이면 deny | 복잡한 wrapper 옵션, 난독화, 파이프·체인·치환 낀 명령, 일반 폴더 삭제, 플러그인 밖 실행 |
...
- **범용 shell parser가 아닙니다.** bare 단일 command + argv와 옵션 없는 제한된 단순 wrapper만 유한 grammar로 파싱합니다.
...
- **`bypassPermissions` 예외:** 사용자가 전 리스크를 인수한 이 permission mode에서는 안전 게이트가
  완전히 stand down하며, 파괴 명령과 MCP를 포함해 어떤 판정도 출력하지 않습니다.
...
### 0.3.0 (2026-07-22)
- **`bypassPermissions` 예외** — 사용자가 전 리스크를 인수한 이 permission mode에서는 안전 게이트가
  완전히 stand down한다(파괴 명령·MCP 포함 어떤 판정도 안 냄).
- **안전 게이트 wrapper 정규화** — 선행 env assignment(`FOO=bar …`)와 옵션 없는 `env`·`command`·`sudo`
  prefix를 벗긴 뒤 파괴 명령을 판정한다(`sudo rm -rf /` 등). 복잡한 wrapper 옵션·`command -v` 같은
  비실행 형태는 판정 없이 위임.
- **부트스트랩 min_version 검증** — installer `min_version`을 semver로 비교해 구버전 CLI를 재설치 대상으로
  판정. 설치돼 있지만 구버전이면 `upgrade_argv`(brew/scoop/winget)로 in-place 업그레이드.
- 부트스트랩 상태머신을 `plan → approve → apply`로 정합 — 승인 판정을 실행 이전으로 옮기고 미승인
  단계 전체를 한 번에 노출, 전부 승인 전엔 아무것도 실행하지 않는다.
- 버전 파싱을 `version_check` stdout 패턴에 anchor(업데이트 배너 semver 오인 방지), manifest 간
  `min_version` 균일 강제. manifest revision 2, PRD rev 12, 테스트 160.
...
- **npm 배포 채널 개통** — [`claude-novice`](https://www.npmjs.com/package/claude-novice)
  (MIT, 플러그인 런타임 37파일). GitHub marketplace와 버전 동기.
...
|---|---|---|
| Local destructive commands (`rm -rf ~`·`/`·project root, `dd`/`mkfs`/`shred`, PowerShell `Format-Volume`/`Clear-Disk`) | parse bare commands and limited simple prefixes (`NAME=value`, `env`, `command`, `sudo`); deny if catastrophic | Complex wrapper options, obfuscation, piped/chained/substituted commands, deleting a normal folder, execution outside the plugin |
...
- **Not a general shell parser.** It parses only a single bare command + argv and limited option-free wrappers. Unsupported
...
- **`bypassPermissions` exception:** in this mode the user assumes the full risk and the safety
  gate stands down completely, emitting no decision even for destructive Bash or MCP calls.
...
- **`bypassPermissions` exception** — in this mode the user assumes full risk and the safety gate
  stands down completely (no decision even for destructive Bash or MCP calls).
- **Safety-gate wrapper normalization** — leading env assignments (`FOO=bar …`) and option-free
  `env`·`command`·`sudo` prefixes are stripped before destructive-command analysis (`sudo rm -rf /`,
  etc.). Complex wrapper options and non-executing forms like `command -v` are delegated instead.
- **Bootstrap min_version enforcement** — an installer's `min_version` is compared as semver, so an
  outdated CLI is treated as install-needed; when installed-but-outdated it upgrades in place via
  `upgrade_argv` (brew/scoop/winget).
- Bootstrap state machine aligned to `plan → approve → apply` — approval is decided before execution
  and all missing approvals are surfaced at once; nothing runs until every step is approved.
- Version parsing anchored to the `version_check` stdout pattern (no mistaking an update-banner
  semver), and `min_version` forced uniform across installers. Manifest revision 2, PRD rev 12, 160 tests.
...
- **npm distribution channel opened** — [`claude-novice`](https://www.npmjs.com/package/claude-novice)
  (MIT, 37 runtime files). Version-synced with the GitHub marketplace.
