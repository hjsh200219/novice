...
| 위협 | 동작 | 보증하지 않는 범위 |
|---|---|---|
| 로컬 파괴 명령 (`rm -rf` 등) | 유한 grammar 해석 후 deny/ask | 난독화, 새 명령, 플러그인 밖 실행 |
...
- **범용 shell parser가 아닙니다.** 단일 command + argv 유한 grammar만 해석합니다.
  pipe·리다이렉트·명령 치환 등 미지원 문법은 위험 token이 섞이면 deny, 아니면 ask로 낮춥니다.
...
외부 runtime dependency 없음 (Node >= 18, node:test).
