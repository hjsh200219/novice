...
2. **강제 안전은 `PreToolUse`에서만 수행한다.** 긍정 탐지된 파괴 비가역·시크릿은 텍스트 권고가 아니라 `permissionDecision: deny`로 막는다(deny-only, ask 티어 없음). 그 밖은 CC 네이티브 권한에 위임한다.
...
| 미지원 문법 위임 | 미지원 benign corpus 100% allow(위임) | 파싱 불가 문법은 deny/ask 없이 CC 네이티브 권한에 위임; 탐지율 분모에서 숨기지 않음 |
...
|---|---|---|---|---|
| 용어 설명 | 세션 첫 노출부터 임계값까지 병기 | 임계값(3회)까지 병기 | 요청 시만 설명 | novice 설명 없음 |
...
- persistent mute는 prompt 전체가 `novice mute <term|alias>`와 일치할 때 해당 용어를 **프로젝트 override의 `muted_terms`**에 추가해 노출 횟수·reset과 무관하게 항상 faded로 취급한다(설명 영구 중단). `novice unmute <term|alias>`로 해제하며, 사전에 없는 용어는 무시한다. reset은 카운터를 0으로 되돌려 다시 설명하게 하고, mute는 즉시·지속적으로 설명을 끊는 점에서 다르다. **mute는 프로젝트 스코프라 세션이 바뀌어도 유지된다**(교차 세션 지속). session-scoped term 카운터와 달리 `${CLAUDE_PLUGIN_DATA}/projects/<hash>.json`에 저장되며, mode 전환에도 보존된다.
...
| 시크릿 deploy | deploy tool/CLI 인자와 대상 파일 scan | known pattern이면 deny | 이미 원격에 저장된 값, 모델 provider·transcript 노출 |
...
- fixture corpus는 지원 grammar dangerous(deny), 지원 grammar benign(allow), 미지원 benign(위임=allow)을 분리한다.
...
- destructive operation은 production과 unknown에서 deny한다. preview/staging·local development 대상은 판정하지 않고 위임한다(ask 티어 없음). 분류 불가한 외부 write는 guided manual로 안내한다.
...
- 프로젝트별 override는 `${CLAUDE_PLUGIN_DATA}/projects/<project-root-hash>.json`에 저장한다. `level`, `enabled`, `protected_branches_extra`와 함께 **교차 세션 지속이 필요한 `muted_terms`**를 여기에 둔다.
...
| `PreToolUse` | 파괴·시크릿 gate (deny-only) | `permissionDecision`의 deny |
...
- reset, mute/unmute, session cleanup, 사전 밖 용어, code block 오탐 fixture가 통과한다. mute된 용어는 노출 횟수·reset과 무관하게 capsule faded 목록에 포함된다.
