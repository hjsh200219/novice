...
- persistent mute는 prompt 전체가 `novice mute <term|alias>`와 일치할 때 해당 용어를 세션 `muted_terms`에 추가해 노출 횟수·reset과 무관하게 항상 faded로 취급한다(설명 영구 중단). `novice unmute <term|alias>`로 해제하며, 사전에 없는 용어는 무시한다. reset은 카운터를 0으로 되돌려 다시 설명하게 하고, mute는 즉시·지속적으로 설명을 끊는 점에서 다르다. mute는 세션 상태에 저장되며 같은 session resume 시 유지된다.
