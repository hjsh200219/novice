...
date: 2026-07-20
revision: 5
mode: adversarial review cross-checked (SDK 타입 정의 검증 반영, 단일 구현 단계)
owner: planner
...
2. **Automate under guardrails** — 외부 서비스 설정을 CLI > 신뢰된 MCP > Claude in Chrome 순서로 검토하되, model-blind secret 경로와 deterministic adapter가 검증된 작업만 자동 실행한다. 나머지는 guided manual로 낮춘다.
...
5. **MVP 외부 자동화는 Vercel 한 개로 제한한다.** secret broker와 adapter contract가 실제로 작동하는지 한 provider에서 먼저 증명한다. Supabase와 GitHub OAuth는 이 경계를 재사용하는 P1이다.

### 검토한 선택지

| 선택지 | 구성 | 장점 | 단점 | 판정 |
|---|---|---|---|---|
| **A. Hook + deterministic runner 단일 플러그인** | hook 기반 학습·안전층 + model-blind secret broker + provider adapter | 기존 output style 무침해, 안전과 학습 상태 분리, 검증 가능한 외부 설정 | hook 실패 경계와 runner 보안·호환성을 직접 관리해야 함 | **채택** |
...
- **Decision:** Hook + deterministic runner 단일 플러그인.
- **Safety seam:** `PreToolUse`는 플러그인이 활성화된 동안 novice level/off와 무관하게 동작한다.
- **Learning seam:** `SessionStart`·`UserPromptSubmit`·`UserPromptExpansion`·`Stop`은 novice mode와 세션별 용어 카운터를 담당한다. 반복 heuristic은 `PostToolUse`(성공)와 `PostToolUseFailure`(실패)가 per-call로 담당한다. batch 단위 hook(`PostToolBatch`류)은 플랫폼에 존재하지 않으므로 병렬 tool 결과는 atomic single-writer로 증분 집계한다.
...
- **Secret seam:** provider secret은 model prompt·tool argv·stdout을 통과시키지 않는다. model-blind broker가 검증되지 않은 환경에서는 해당 작업을 guided manual로 낮춘다.
...
| 2 | 외부 서비스 설정을 AI가 대신, CLI > MCP > Chrome | Vercel adapter 1개 + model-blind secret broker; 나머지는 guided manual/P1 | 최소 명세, E2E 미검증 |
...
> 요구 2의 “임의 서비스 자동화”는 MVP 범위가 아니다. MVP는 Vercel의 검증된 작업만 자동화한다. Supabase와 GitHub OAuth는 secret broker·adapter contract를 재사용하는 P1이다.
>
...
- Vercel 설정을 model-blind secret 경로와 human-in-the-loop 흐름으로 완료한다.
...
| Vercel 설정 완수율 | 참가자 80% 이상이 추가 secret 노출 없이 완료 | 로그인 시작부터 `vercel project inspect`·env 이름 검증까지; guided manual 사용도 별도 기록 |
| 핵심 용어 이해 | 즉시 및 24시간 후 baseline보다 높은 정답률 | `commit`·환경변수·preview deployment의 동형 문항, 효과크기와 CI 보고 |
| 지원 문법 위험 fixture 탐지율 | 100% | 단위·통합·mutation fixture; 지원 밖 문법은 탐지율 분모가 아니라 deny/ask 정책으로 검증 |
...
- mode 변경은 현재 프로젝트에 지속된다. `/novice:mode`는 `UserPromptExpansion`이, 명시적 자연어 별칭은 `UserPromptSubmit`이 capsule 주입 전에 state를 갱신하므로 두 경로 모두 해당 turn부터 반영된다. `off` 전환 turn에는 OFF tombstone만 주입하고 이후 turn부터 novice context를 주입하지 않는다.
...
- `Stop` hook이 `last_assistant_message`(SDK상 optional — 부재 시 카운터를 갱신하지 않고 종료)에서 canonical “용어+설명” 패턴만 세어 세션 카운터를 갱신한다. 한 최종 응답에서 같은 용어는 여러 번 나와도 1회만 증가시키고, `prompt_id`와 응답 hash로 중복 처리를 막는다. 사용자 입력, 코드 블록, 설명 없는 단순 노출은 카운트하지 않는다. 사용자 interrupt와 중간 assistant message는 P0 카운터에 반영되지 않는 한계로 공개한다.
...
- `UserPromptSubmit`은 off가 아닐 때만 800자 이하의 현재 mode capsule을 주입한다. glossary는 매 turn 반복하지 않는다.
...
### 4.3 Vercel 설정 자동화와 secret 경계 [P0]

#### 공통 원칙

- MVP provider는 Vercel 한 개다. “CLI > MCP > Chrome”은 서비스 전체가 아니라 **개별 작업 capability**마다 적용한다.
- adapter는 `detect`, `plan`, `approve`, `apply`, `verify`, `recover` 단계와 작업별 `cli`, `mcp`, `chrome`, `manual` 지원 여부를 코드로 선언한다. `SKILL.md`만으로 deterministic adapter를 구현했다고 간주하지 않는다.
- CLI가 설치돼 있다는 이유만으로 그 CLI가 지원하지 않는 작업을 강행하지 않는다.
- 로그인, CAPTCHA, MFA, 약관 동의는 사용자가 직접 완료한다. Claude Code의 `Bash` tool에는 사용자용 stdin channel이 없다고 가정하며, 토큰·비밀번호를 대화창·tool argument·shell history에 붙여넣도록 요구하지 않는다.
...
- 브라우저 자동화는 visible mode로만 수행한다. 최종 Create/Delete/Deploy submit 직전 정지는 model 지시만으로 강제할 수 없으므로 P0에서는 advisory로 표시하고, enforcement가 필요한 작업은 CLI adapter 또는 guided manual만 허용한다.
- MCP는 서비스 제공자가 공식 배포하고, 실제 runtime에서 식별 가능한 server·tool schema·권한·transport만 `service-capabilities.json` allowlist에 고정한다. provenance를 hook input에서 검증할 수 없으면 allowlist 자동 실행 대상에서 제외한다.
- allowlist MCP도 첫 사용 전 요청 scope와 전송 대상 host를 보여 주고, read-only probe를 먼저 수행한다.

#### model-blind secret broker

- provider secret이 필요한 작업은 `bin/novice-secret`과 `adapters/vercel` runner를 통해서만 자동화한다.
- `novice-secret stage <label>`은 Claude Code 밖의 사용자 terminal에서 hidden input을 받고 OS keychain에 1회용 handle로 저장한다. handle만 대화에 들어올 수 있으며 secret 값은 prompt·argv·stdout·debug output에 들어가지 않는다.
- runner는 handle로 secret을 읽어 `spawn`의 stdin으로 Vercel CLI에 전달하고, provider·변수 이름·environment·성공 여부만 반환한 뒤 handle을 폐기한다. shell string과 `echo secret | ...`를 사용하지 않는다.
- OS keychain adapter가 없거나 broker contract test가 실패하면 env 작업은 guided manual로 낮춘다. plaintext fallback, plugin data의 secret 저장, secret 값을 포함한 자동화는 제공하지 않는다.
- `PostToolUse`의 `updatedMCPToolOutput` redaction은 **MCP tool 출력에만** 적용된다. Bash로 실행되는 CLI 출력에는 적용할 수 없으므로 CLI 경로의 방어는 runner의 출력 정제뿐이다. 어느 쪽도 secret이 이미 model input이나 command argument에 들어간 것을 되돌릴 수 없으므로 broker를 대체하지 않는다.

#### 작업별 capability matrix

| Vercel 작업 | 1순위 | 2순위 | 3순위 | 최종 검증 |
|---|---|---|---|---|
...
| Vercel env 설정 | broker + CLI | 검증된 MCP elicitation | guided manual | 이름·environment 확인, 값 미출력 |
| preview deploy | CLI adapter + 직전 ask | guided manual | — | preview URL health check |
...
4. 환경변수는 preview/development/production 범위를 분리한다. secret 값은 broker handle로 전달하고 화면·model context·shell history에 출력하지 않는다.
...
- Supabase는 현행 `publishable`/`secret` key 모델과 broker를 사용한다. legacy `anon`/`service_role` 중심 설계를 새로 만들지 않는다.
- GitHub OAuth App 생성은 현재 공식 UI/Chrome/manual 경로를 기본으로 본다. `gh api` 생성 지원을 검증 없이 약속하지 않는다.
- GitHub OAuth App은 callback URL을 하나만 지원하므로 local·preview·production을 동시에 직접 등록한다고 약속하지 않는다. P1 착수 전에 환경별 app 분리, 안정된 callback proxy, 또는 GitHub App 전환 중 하나를 ADR로 결정한다.
- authorization round trip은 callback server와 framework adapter가 준비된 provider에서만 acceptance 항목으로 둔다.
...
| 비용·반복 루프 | `PostToolUse`·`PostToolUseFailure`가 per-call로 갱신하는 세션 fingerprint | 반복 작업 휴리스틱에서 novice 개입(`additionalContext`) | 정확한 과금 계산·hard billing cap |
...
- `claude -p`에서 사용자 입력이 필요한 작업은 기본적으로 guided manual 또는 deny로 낮춘다. hook input의 `permission_mode`만으로 비대화형 여부를 단정하지 않는다. `permissionDecision`의 `defer` 값은 버전별 의미가 교차 검증되지 않았으므로 P0에서 사용하지 않는다.
- 안전 hook의 JSON 오류·예외·지원 밖 문법·입력 상한 초과는 exit 2/deny로 처리한다. 플랫폼이 timeout·강제 종료를 non-blocking으로 처리하는 경우는 제품 보증 범위에서 제외하고, scan 전에 크기 상한을 검사한다.
...
novice/
  ...
  adapters/
    vercel.js                 # detect/plan/approve/apply/verify/recover
    supabase.js               # P1
    github-oauth.js           # P1
  bin/
    novice-secret              # model-blind keychain broker
  scripts/
    session-start.*
    user-prompt-submit.*
...
| `UserPromptExpansion` | `command_name`(`novice:mode`) 매칭으로 plugin 명령 판별 후 `command_args`를 deterministic하게 처리 | project state 갱신. expansion 차단은 불가 — invalid args는 state 미변경 + `additionalContext` 안내 |
| `PreToolUse` | 파괴·시크릿·외부 부작용 gate | `permissionDecision: deny|ask` |
| `PostToolUse` | 성공 tool fingerprint 증분 갱신 + MCP tool output redaction | session state 갱신, `updatedMCPToolOutput`(MCP 전용), 임계값 초과 시 `additionalContext` 개입 |
| `PostToolUseFailure` | 실패 tool fingerprint와 오류 유형 증분 갱신 | 반복 실패 시 `additionalContext` 개입 문구 |
...
- `plugin.json`, `hooks/hooks.json`, 전체 hook scripts, `config/*.json` schema, `adapters/vercel`, `bin/novice-secret`, `skills/`, `tests/` 전부.
- Claude Code 최소 지원 version과 공식 문서·SDK 타입 snapshot 링크.

### 완료 기준 A — 플랫폼 contract

- plugin skill이 `/novice:mode` namespace로 노출된다.
- `UserPromptExpansion`에서 공식 payload인 `expansion_type`, `command_name`, `command_args`, `command_source`(optional), `prompt`를 받아 `command_name` namespace(`novice:mode`)를 매칭하고 state를 갱신한다. 실제 설치 버전에서 payload fixture를 캡처하고 최소 지원 버전을 문서화한다.
- **expansion 차단은 플랫폼상 불가능하다** (출력 스키마는 `additionalContext`뿐). invalid args는 state를 변경하지 않고 `additionalContext`로 사용 가능한 값 안내를 주입하며, 모델이 이를 사용자에게 표시하는 fixture로 검증한다.
...
- invalid args는 state 변경 없이 `additionalContext` 안내가 주입되고 사용 가능한 값만 보여 준다.
...
- `Stop.last_assistant_message`로 용어+설명 노출만 count하고 전체 transcript는 재파싱하지 않는다. 필드 부재(optional) 시 카운터를 건드리지 않는다.
...
- hook 예외·잘못된 JSON·지원 밖 shell 문법·oversized diff는 deny/ask로 낮아지고, timeout 한계는 보증 범위에 표시된다.
...
- 성공·실패가 섞인 병렬 tool 결과와 반복 수정 fixture에서 `PostToolUse`·`PostToolUseFailure` fingerprint가 atomic하게 집계되고, 임계값을 넘으면 개입 문구가 주입되며, novice off에서는 주입되지 않는다.

### 완료 기준 D — Vercel E2E (MVP)
...
- Vercel acceptance scenario가 clean sandbox 계정에서 성공한다.
- secret broker가 secret을 model prompt·argv·stdout·debug output에 노출하지 않는 contract test를 통과한다.
- 재실행해도 중복 project/env를 만들지 않는 idempotency test가 통과한다.
- 중간 실패 fixture마다 remote/local partial state와 다음 복구 행동을 보고한다.
- Supabase와 GitHub OAuth는 P1 ADR과 adapter contract 승인 후에만 착수한다.
...
| model secret 노출 | credential 유출 | OS keychain broker, secret 없는 argv/stdout, broker 실패 시 guided manual |
...
| 반복 실패·병렬 state race | 비용 증가·counter 손상 | `PostToolUse`·`PostToolUseFailure` per-call 증분 집계, 단일 writer lock과 concurrency fixture |
...
- P0 external service는 Vercel 한 개다. Supabase와 GitHub OAuth는 P1이다.
...
- Vercel secret broker의 OS별 keychain 지원 범위와 guided manual 전환 기준.
...
| hook payload 타입 확정: `UserPromptExpansion` 필드·차단 불가, `PostToolUseFailure` 존재, `PostToolBatch` 부재, `updatedMCPToolOutput`(MCP 전용), `Stop.last_assistant_message`(optional) | `@anthropic-ai/claude-agent-sdk` v0.2.117 `sdk.d.ts` 타입 정의 | §5 hook 책임, §6 완료 기준 A·C 교정 |
