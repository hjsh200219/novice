...
`/novice:focus`(응답 형태 규칙)는 **별개 다이얼**이다. level을 바꿔도 focus는 그대로고,
`/novice:mode off`로 학습층을 꺼도 focus는 계속 동작한다. 끄려면 `/novice:focus off`.

두 지시가 충돌하면 focus가 이긴다(서론·사후 요약·행동 해설 억제). 단 **용어 괄호 병기는
유지**된다 — `commit(…)`은 서론이 아니라 인라인이기 때문이다. 상세: focus 스킬.
