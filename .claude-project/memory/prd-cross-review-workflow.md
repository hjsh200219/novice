...
`/Users/hoshin/workspace/SHC/novice`는 비개발자 입문자용 Claude Code "novice" 플러그인 프로젝트다. 2026-07-20 기준 docs/PRD.md만 있는 문서 단계이며, PRD는 revision 8, status는 여전히 `pending approval`, 구현 미시작. 구현은 단계 분할 없이 전체 일괄 개발로 결정됨(rev5). 외부설정은 secret 미취급 **2-tier CLI 부트스트랩**(rev8 절충, 사용자 결정): Tier 1 검토 manifest(vercel/gh/supabase) 자동, Tier 2 그 외 CLI는 공식 근거 제시 + 사용자 승인 후 동일 engine — 개수 제한 없음.
